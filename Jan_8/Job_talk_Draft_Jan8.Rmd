...
class: inverse 
## Is warfare is a product of recent human societies or does it have a longer chronology?
...
.pull-right[
![](https://images-na.ssl-images-amazon.com/images/I/61lMFCoeziL._SX331_BO1,204,203,200_.jpg)
]
---
...
# "If you lived 10,000 years ago and were a man “there was about a one in four chance that you would die violently”  -Michael Shermer. 


.pull-left[
![](images/Shermer_2.png)
]

.pull-right[
![](images/Shermer_3.jpg)
]
---
...
![](http://cdn.static-economist.com/sites/default/files/lead_2.jpg)
...
![](https://thenypost.files.wordpress.com/2017/08/170810-hunting-carved-bone-story-feature.jpg)
...
![](https://ourworldindata.org/wp-content/uploads/2013/08/Share-of-Deaths_Archaeological-and-Eth notesnographic-Evidence-on-Violence.png "world in data")
...
```{r datatable for war and read in data, warning=FALSE, message= FALSE, echo=FALSE}
...
```{r data for interactive DCI, echo=FALSE, message=FALSE}
DCI <- read_csv("X_Factor_Dataset.csv")
DCI_interact <-  DCI %>% gather(value = X, key = country, Belgium:Iraq)
DCI_summary <- DCI_interact %>% group_by(country) %>% summarise(DCI_mean = mean(X, na.rm = TRUE), DCI_sd = sd(X, na.rm = TRUE))
#setup data
...
```{r CC data table, eval=FALSE, error=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r datatable, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, }
cc <-read_csv("CC_data_Updated_4.csv") #read in data
...
background-image: url(https://pbs.twimg.com/media/DZ0tfpXW4AAcKci.jpg)
background-position: center
background-repeat: no-repeat
background-size: contain
